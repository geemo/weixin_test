...
/* http请求
 * @param {String|Object} opts 如果是对象支持url,form,datatype,apitype,headers字段，如果是字符串就代表url
...
function request(opts, form) {
    return new Promise((resolve, reject) => {
        ...
        if(protocol === 'http:') {
            ...
            return reject(`Protocol "${protocol}" not supported`);
        }

        if(method === 'GET') {
            http_method.get(url, callback);
        } else if(method === 'POST') {
            ...
            if(file_path) {
                ...
                fs.stat(file_path, (err, stat) => {
                    ...
                    formStream.pipe(http_method.request(options, callback))
                ...
                const req = http_method.request(options, callback);
        ...
        function callback(res) {
            ...
            const apitype = opts.apitype || 'wechat';
            ...
            res.on('end', () => {
                ...
                if (apitype === 'wechat') {
                    if ((data.errcode || data.errmsg) && data.errcode !== 0)
                        ...
                        resolve(data);
                } else if(apitype === 'kugou') {
                    if ((data.status && data.status !== 1) || data.error || data.errorcode !== undefined) 
                        reject(data);
                    else 
                        resolve(data);
                } else {
